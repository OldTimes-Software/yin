...
static void Act_DrawBasic( Actor *self, void *userData )
{
	...
	YR_DrawAxesPivot( Act_GetPosition( self ), PLVector3( 0, 0, 0 ) );
}
...
bool Act_IsVisible( Actor *self, YRCamera *camera )
{
	if ( camera == NULL )
		return false;
...
void Act_DrawActors( YRCamera *camera, WorldSector *sector )
{
...
#	if 1
			...
			{
				WorldFace *face = PlGetLinkedListNodeUserData( colliderNode );
				...
				if ( collision.penetration > 0.0f )
				{
					...
					YR_DrawAxesPivot( collision.contactPoint, plane.normal );
...
void Act_TickActors( void *userData, double delta )
{
	...
	while ( index != NULL )
	{
		...
		if ( actor->sector != NULL )
		{
			...
			WorldFace  **faces = WorldSector_GetMeshFaces( actor->sector, &numFaces );
