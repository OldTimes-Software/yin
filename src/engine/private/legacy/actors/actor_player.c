...
YRCamera *Player_GetCamera( Actor *self )
{
...
static void Player_ApplyViewBob( Actor *self )
{
	...
	APLAYER( self )->viewBob += ( sinf( Engine_GetNumTicks() / 5.0f ) / 10.0f ) * velocityVector;

	float viewOffset = self->position.y + PLAYER_VIEW_OFFSET;
	if ( OS_Shell_GetKeyState( 'c' ) )
...
static void Player_Tick( Actor *self, void *userData )
{
	...
	float maxVelocity = OS_Shell_GetButtonState( INPUT_LEFT_STICK ) || OS_Shell_GetKeyState( KEY_LEFT_SHIFT ) ? PLAYER_RUN_SPEED : PLAYER_WALK_SPEED;
...
static void Player_Draw( Actor *self, void *userData )
{
	...
	for ( unsigned int i = 0; i < APLAYER( self )->model->numMeshes; ++i )
		YinCore_Material_DrawMesh( RM_GetFallbackMaterial(), APLAYER( self )->model->meshes[ i ], NULL, 0 );
