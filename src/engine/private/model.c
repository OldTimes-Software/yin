...
static void DestroyModel( void *userData )
{
	...
	if ( additionalData != NULL )
	{
		...
		{
			YinCore_Material_Release( additionalData->materials[ i ] );
		}
	}
...
static PLMModel *DeserializeModel( NLNode *root )
{
	...
	if ( materialArray == NULL )
	{
		...
		userData.materials[ 0 ] = YinCore_Material_Cache( "materials/engine/fallback_mesh.mat.n", 0, true, false );
	}
	else
	{
		...
		for ( unsigned int i = 0; i < userData.numMaterials; ++i )
		{
			...
			{
				userData.materials[ i ] = YinCore_Material_Cache( "materials/engine/fallback_mesh.mat.n", 0, false, false );
			...
			{
				userData.materials[ i ] = YinCore_Material_Cache( materialPath, 0, true, false );
			}
