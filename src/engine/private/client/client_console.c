...
static void ToggleConsole( void )
{
	...
	if ( mouseLook != NULL && mouseLook->b_value )
		OS_Shell_GrabMouse( !consoleIsOpen );
}
...
bool Client_Console_HandleKeyboardEvent( int key, unsigned int keyState )
{
	...
	if ( keyState != INPUT_STATE_PRESSED && keyState != INPUT_STATE_DOWN )
	{
		return true;
	}
...
static void Client_Console_DrawInputField( const YRViewport *viewport )
{
...
#define SPACER 4.0f
	...
	if ( v < Engine_GetNumTicks() )
		v = Engine_GetNumTicks() + 20;

	char c = ( v > Engine_GetNumTicks() + 10 ) ? '_' : ' ';
 ...
 */
void Client_Console_Draw( const YRViewport *viewport )
{
	if ( !consoleIsOpen )
		return;
...
#define CON_INPUT_COLOUR     PLColour( 0, 0, 0, 255 )
	...
	/* draw version info */
	{
		...
		static char buf[ 32 ]  = "v" ENGINE_VERSION_STR " [" GIT_BRANCH "." GIT_COMMIT_COUNT "]";
...
static void CreateViewportCommand( unsigned int argc, char **argv )
{
	...
	if ( !OS_Shell_CreateWindow( "Yin Viewport", width, height, false, 0 ) )
	{
