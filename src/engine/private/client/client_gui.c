...
void Client_GUI_Initialize( void )
{
	...
	YRRenderTarget *guiTarget = YR_RenderTarget_Create( "gui", 640, 480, PLG_BUFFER_COLOUR | PLG_BUFFER_DEPTH );
	if ( guiTarget == NULL )
	{
		PRINT_ERROR( "Failed to create default render target for GUI!\n" );
	}

	baseGuiMat = YinCore_Material_Cache( "materials/ui/ui_rt_base.mat.n", CACHE_GROUP_WORLD, false, false );
	if ( baseGuiMat == NULL )
	{
...
void Client_GUI_Draw( const YRViewport *viewport )
{
	...
	PLGTexture *texture;
	if ( ( texture = YR_GetPrimaryColourAttachment() ) != NULL )
	{
	...
	PROFILE_START( PROFILE_DRAW_GUI );
	if ( drawGUI )
	{
		...
		YR_Draw2DQuad( baseGuiMat, 0, 0, viewport->width, viewport->height );
	}
	PROFILE_END( PROFILE_DRAW_GUI );
	...
	{
		char tmp[ 32 ];
		snprintf( tmp, sizeof( tmp ), "FPS: %u", YinCore_Viewport_GetAverageFPS( viewport ) );
