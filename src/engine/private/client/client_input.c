...
typedef struct ClientInputAction
{
	...
	ClientInputButton buttons[ MAX_BUTTON_INPUTS ];
	unsigned int numButtonBinds;

	ClientInputKey keys[ MAX_KEY_INPUTS ];
...
static struct
{
	...
	int ox, oy;
	int dx, dy;
	OSInputState buttons[ MAX_CLIENT_INPUT_MOUSE_BUTTONS ];
...
static struct
{
	OSInputState keys[ MAX_KEY_INPUTS ];
} inputKeyboard;

typedef struct ClientInputController
{
	bool isActive;
	OSInputState buttons[ MAX_BUTTON_INPUTS ];
...
static void IterateAction( void *userData, bool *breakEarly )
{
	...
	{
		OSInputState state = OS_Shell_GetButtonState( action->buttons[ i ] );
		if ( ( state != INPUT_STATE_DOWN ) && ( state != INPUT_STATE_PRESSED ) )
			continue;
	...
	{
		OSInputState state = OS_Shell_GetKeyState( action->keys[ i ] );
		if ( ( state != INPUT_STATE_DOWN ) && ( state != INPUT_STATE_PRESSED ) )
			continue;
                                  ...
                                  ClientInputActionCallback actionCallback )
{
	...
	if ( numDefaultButtons > MAX_BUTTON_INPUTS )
	{
		numDefaultButtons = MAX_BUTTON_INPUTS;
		PRINT_WARNING( "Too many default button inputs for action!\n" );
	}
	if ( numDefaultKeys > MAX_KEY_INPUTS )
	{
		numDefaultKeys = MAX_KEY_INPUTS;
	...
	memcpy( inputAction->buttons, buttons, sizeof( ClientInputButton ) * numDefaultButtons );
	inputAction->numButtonBinds = numDefaultButtons;

	memcpy( inputAction->keys, keys, sizeof( ClientInputKey ) * numDefaultKeys );
...
void Client_Input_HandleKeyboardEvent( int key, OSInputState keyState )
{
	if ( Client_Console_HandleKeyboardEvent( key, keyState ) )
		return;
}

void Client_Input_HandleMouseButtonEvent( int button, OSInputState buttonState )
{
	GUI_UpdateMouseButton( button, ( buttonState == INPUT_STATE_DOWN ) );

	if ( buttonState != INPUT_STATE_RELEASED && ( inputMouse.buttons[ button ] == INPUT_STATE_PRESSED || inputMouse.buttons[ button ] == INPUT_STATE_DOWN ) )
		return;
...
static bool GetSDLButtonState( SDL_GameController *gameController, ClientInputButton button )
{
...
void Client_Input_Tick( void )
{
	...
	for ( unsigned int i = 0; i < CLIENT_INPUT_MAX_CONTROLLERS; ++i )
	{
		...
		for ( unsigned int j = 0; j < MAX_BUTTON_INPUTS; ++j )
		{
			bool state = GetSDLButtonState( controllers[ i ].sdlGameController, j );
			if ( !state )
			{
				controllers[ i ].buttons[ j ] = INPUT_STATE_NONE;
				continue;
			}

			if ( controllers[ i ].buttons[ j ] == INPUT_STATE_DOWN )
				continue;

			controllers[ i ].buttons[ j ] = ( controllers[ i ].buttons[ j ] == INPUT_STATE_PRESSED ) ? INPUT_STATE_DOWN : INPUT_STATE_PRESSED;
		}
...
void Client_Input_EndFrame( void )
{
	...
	int w, h;
	OS_Shell_GetWindowSize( &w, &h );
	OS_Shell_SetMousePosition( w / 2, h / 2 );
}

unsigned int Eng_Cli_Input_GetNumControllers( void ) { return numControllers; }
