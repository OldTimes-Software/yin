...
static void *ParseWav( PLFile *file, AudioWaveFormat *waveFormatEx, unsigned int *bufferSize )
{
	...
	{
		if ( chunkSize <= sizeof( AudioWaveFormat ) )
		{
			PRINT_WARNING( "Chunk size is too small to hold format data!\n" );
			return NULL;
		}

		if ( !ReadChunkData( file, waveFormatEx, sizeof( AudioWaveFormat ), chunkPosition ) )
			return NULL;
	}
	else
		return NULL;
