...
static ShaderProgramIndex *bloomFilterShader;
static ShaderProgramIndex *bloomBlurXShader;
static ShaderProgramIndex *bloomBlurYShader;
...
{
	bloomFilterShader = RS_GetShaderProgram( "post_bloom_filter" );
	if ( bloomFilterShader == NULL )
		return false;
	bloomBlurXShader = RS_GetShaderProgram( "post_blur_x" );
	if ( bloomBlurXShader == NULL )
		return false;
	bloomBlurYShader = RS_GetShaderProgram( "post_blur_y" );
...
static void DrawBloomEffect( const YRViewport *viewport )
{
	...
	YR_SetupRenderTarget( &bloomBuffer, &bloomTexture, NULL, bw, bh );
	...
	PlgDrawTexturedRectangle( viewport->x, viewport->height, bw, -bh, YR_GetPrimaryColourAttachment() );
