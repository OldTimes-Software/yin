...
void YR_SetupDefaultState( const YRViewport *viewport )
{
	...
	if ( world != NULL && ( viewport->camera == NULL || viewport->camera->mode == YR_CAMERA_MODE_PERSPECTIVE ) )
	{
...
void YR_BeginDraw( YRViewport *viewport )
{
	...
	if ( viewport->perf.frameIndex >= YR_MAX_FPS_READINGS )
	{
		viewport->perf.frameIndex = 0;
	}
	viewport->perf.oldTime = newTime;

	YR_SetupDefaultState( viewport );
...
static Material *ppFXAAMaterial = NULL;
...
void YR_Initialize( void )
{
	...
	YR_RenderTarget_Initialize();
	YR_Material_Initialize();
	YR_Font_Initialize();
...
void YR_Shutdown( void )
{
	...
	YR_RenderTarget_Shutdown();
}

/**
 * Where the magic of post processing happens.
 */
static void DrawScenePost( const YRViewport *viewport )
{
...
static void DrawEditorOverlay( const YRViewport *viewport )
{
	...
	const YRCamera *camera = viewport->camera;
	if ( camera != NULL && ( camera->mode != YR_CAMERA_MODE_PERSPECTIVE ) && ( editorInstance->gridScale > 0 ) )
	{
		PlgSetShaderProgram( defaultShaderPrograms[ RS_SHADER_DEFAULT_VERTEX ] );
		...
		YR_World_DrawWireframe( Game_GetCurrentWorld(), &tmp );
...
static void DrawDebugOverlay( const YRViewport *viewport )
{
...
void YR_DrawMenu( const YRViewport *viewport )
{
	...
	YR_Set2DViewportSize( viewport->width, viewport->height );
...
static void YR_RenderScene( YRCamera *camera, const YRViewport *viewport )
{
	WorldSector *currentSector = NULL;
	World       *world         = Game_GetCurrentWorld();
	if ( world != NULL )
	{
		currentSector = World_GetSectorByGlobalOrigin( world, &camera->internal->position );
	}
	...
	*/
	...
	if ( viewport != NULL )
	{
		if ( camera->mode == YR_CAMERA_MODE_PERSPECTIVE )
		{
			if ( camera->drawMode == YR_CAMERA_DRAW_MODE_WIREFRAME )
			{
				YR_World_DrawWireframe( world, camera );
			}
			else
			{
				YR_World_Draw( world, currentSector, camera );
			}
...
void YR_DrawScene( YRCamera *camera, const YRViewport *viewport )
{
	...
	YR_SetupRenderTarget( &fboBuffer, &colourTexture, &depthTexture, viewport->width, viewport->height );
	...
	if ( ( camera != NULL && camera->drawMode == YR_CAMERA_DRAW_MODE_WIREFRAME ) || wireframeMode->b_value )
	{
