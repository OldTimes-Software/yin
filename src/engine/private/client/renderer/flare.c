...
void Flare_Initialize( void )
{
	for ( unsigned int i = 0; i < MAX_FLARE_TEXTURES; ++i )
		flareTextures[ i ] = RT_LoadTexture( flarePaths[ i ], PLG_TEXTURE_FILTER_LINEAR );
	for ( unsigned int i = 0; i < MAX_SHINE_TEXTURES; ++i )
		shineTextures[ i ] = RT_LoadTexture( shinePaths[ i ], PLG_TEXTURE_FILTER_LINEAR );
}
...
void Flare_RenderFlares( const YRCamera *camera )
{
