...
static ShaderProgramIndex *RS_ParseShaderProgram( NLNode *root )
{
	...
	memset( &program, 0, sizeof( ShaderProgramIndex ) );
	...
	if ( child != NULL )
	{
		...
		RM_ParseMaterialPass( child, &program.defaultPass );
	}

	/* allocate and return our program index */
	ShaderProgramIndex *out = PlMAlloc( sizeof( ShaderProgramIndex ), true );
	*out = program;
	return out;
}
...
ShaderProgramIndex *RS_GetShaderProgram( const char *name )
{
...
void YR_Shader_Initialize( void )
{
	...
	{
		ShaderProgramIndex *programIndex = RS_GetShaderProgram( defaultShaderNames[ i ] );
