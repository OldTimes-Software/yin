...
#if 0
extern "C" void  clisp_init();                      // external initalizer call by lisp_init()
extern "C" long  c_caller( long number, void *arg );// exten c function switches on number
extern "C" void *l_caller( long number, void *arg );// exten lisp function switches on number
extern "C" void *l_obj_get( long number );           // exten lisp function switches on number
extern "C" void  l_obj_set( long number, void *arg );// exten lisp function switches on number
extern "C" void  l_obj_print( long number );         // exten lisp function switches on number
#else // TODO: temporarily shove these here for now, but we'll replace them in the long-term
void clisp_init()
{
}
...
void where_print( int max_lev = -1 )
{
	...
	if ( max_lev == -1 )
		max_lev = ( int ) PtrRef::stack.m_size;
	else if ( max_lev >= ( int ) PtrRef::stack.m_size )
		max_lev = ( int ) PtrRef::stack.m_size - 1;
...
extern "C" void lbreak( char const *format, ... )
{
	break_level++;
...
{
	LString *s = Create( ( long ) strlen( string ) + 1 );
...
{
	LSysFunction *ls = new_lisp_sys_function( min_args, max_args, Callback );
...
LSymbol *new_lisp_symbol( const char *name )
{
...
LSymbol *add_sys_function(char const *name, short min_args, short max_args, LSysCallback Callback )
{
	...
	if ( s->m_function != l_undefined )
	{
		...
		s->m_function = new_lisp_sys_function( min_args, max_args, Callback );
	return s;
}
...
LSymbol *add_c_function( char const *name, short min_args, short max_args, LSysCallback Callback )
{
	...
	if ( s->m_function != l_undefined )
	{
		...
		s->m_function = new_lisp_c_function( min_args, max_args, Callback );
	return s;
}

LSymbol *add_c_bool_fun( char const *name, short min_args, short max_args, LSysCallback Callback )
{
	total_user_functions++;
	...
	if ( s->m_function != l_undefined )
	{
		...
		s->m_function = new_lisp_c_bool( min_args, max_args, Callback );
	return s;
}


LSymbol *add_lisp_function( char const *name, short min_args, short max_args, LSysCallback Callback )
{
	total_user_functions++;
	need_perm_space( "add_c_bool_fun" );
	LSymbol *s = LSymbol::FindOrCreate( name );
	if ( s->m_function != l_undefined )
	{
		...
		s->m_function = new_user_lisp_function( min_args, max_args, Callback );
	return s;
}

static void skip_c_comment( char const *&s )
{
	s += 2;
	while ( *s && ( *s != '*' || *( s + 1 ) != '/' ) )
	{
...
static long str_token_len( char const *st )
{
	long x = 1;
	while ( *st && ( *st != '"' || st[ 1 ] == '"' ) )
	{
...
static int read_ltoken( char const *&s, char *buffer )
{
	// skip space
	while ( *s == ' ' || *s == '\t' || *s == '\n' || *s == '\r' || *s == 26 ) s++;
	if ( *s == ';' )// comment
...
LObject *LSymbol::EvalFunction( void *arg_list )
{
	...
	switch ( t )
	{
		case L_SYS_FUNCTION:
			...
			break;
		case L_L_FUNCTION:
			ret = ( LObject * ) ( ( LSysFunction * ) fun )->callback( arg_list );
			break;
		case L_USER_FUNCTION:
			return EvalUserFunction( ( LList * ) arg_list );
		case L_C_FUNCTION:
		case L_C_BOOL:
		{
			...
			if ( t == L_C_FUNCTION )
				ret = LNumber::Create( ( ( LSysFunction * ) fun )->callback( first ) );
			else if ( ( ( LSysFunction * ) fun )->callback( first ) )
				ret = true_symbol;
			else
				ret = nullptr;
			break;
...
LObject *LSysFunction::EvalFunction( LList *arg_list )
{
	if ( callback == NULL )
	{
		dprintf( "Undefined system function!\n" );
		return NULL;
	}

	PtrRef ref1( arg_list );
	return ( LObject * ) callback( arg_list );
...
void Lisp::Init()
{
	...
	for ( size_t i = 0; i < sizeof( sys_funcs ) / sizeof( *sys_funcs ); i++ )
		add_sys_function( sys_funcs[ i ].name,
		                  sys_funcs[ i ].min_args,
		                  sys_funcs[ i ].max_args,
		                  sys_funcs[ i ].callback );
